...
func TestSelectUnpushedSince_CursorAdvances(t *testing.T) {
	...
	first, err := s.SelectUnpushedSince(ctx, PushCursor{}, 1<<20, "o", "u")
	if err != nil {
	...
	second, err := s.SelectUnpushedSince(ctx, first.Cursor, 1<<20, "o", "u")
	if err != nil {
...
func TestSelectUnpushedSince_BudgetForwardProgress(t *testing.T) {
	...
	batch, err := s.SelectUnpushedSince(ctx, PushCursor{}, 1, "o", "u")
	if err != nil {
		t.Fatalf("SelectUnpushedSince: %v", err)
	}
	if batch.RowCount() != 1 {
