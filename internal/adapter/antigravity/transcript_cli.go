...
// Antigravity conversation's plaintext per-turn trace. Layout:
//
//	<desktopRoot>/brain/<uuid>/.system_generated/logs/overview.txt
//
// where desktopRoot is `<gemini-root>/antigravity` (the parent of the
// conversations/ directory). The file is named `overview.txt` rather
// than `transcript.jsonl` but the wire format is the same JSONL
// schema as CLI (verified 2026-05-24: same `step_index` / `source` /
// `type` / `status` / `created_at` / `content` / `tool_calls` keys,
// same DONE-only finalisation contract).
//
// Why: as of the IDE build active 2026-05-24 on the operator's host,
// desktop Antigravity rotated its .pb encryption cipher to one
// oscrypt can't currently decrypt — the same blocker that gated CLI
// recovery. Plaintext overview.txt is now the reliable on-disk
// transcript for desktop conversations, mirroring the CLI's
// transcript.jsonl. Without this fallback, every fresh desktop
// conversation lands as zero-events-extracted after decrypt fails.
func desktopTranscriptPath(desktopRoot, conversationID string) string {
	return filepath.Join(desktopRoot, "brain", conversationID, ".system_generated", "logs", "overview.txt")
}
