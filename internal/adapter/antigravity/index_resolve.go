...
// AntigravityConversationsDirs returns the conversation directories
// the backfill should walk: every crossmount-resolved home's
// `.gemini/antigravity/conversations`. Mirrors codex's helper for
// symmetry with the dispatcher.
func AntigravityConversationsDirs(homes []string) []string {
	var dirs []string
	for _, h := range homes {
		dirs = append(dirs, h+"/.gemini/antigravity/conversations")
	}
	return dirs
}
