...
// discoverLanguageServers enumerates running language_server_*.exe
// (or language_server_<os>_x64) processes and returns the
// (csrf_token, listening port) pairs needed to reach them via gRPC.
//
...
func discoverNativeLinux() ([]LanguageServer, error) {
	...
	for _, e := range entries {
		...
		if !strings.Contains(strings.ToLower(filepath.Base(exe)), "language_server") {
			continue
		}
...
func discoverNativeMac() ([]LanguageServer, error) {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		// Match language_server_darwin_x64 / arm64
		if !strings.Contains(line, "language_server_") {
			continue
		}
		// First whitespace-separated field is PID, rest is cmd.
		fields := strings.Fields(line)
		if len(fields) < 2 {
			continue
		}
		...
		lsofOut, err := exec.Command("lsof", "-iTCP", "-sTCP:LISTEN", "-a", "-p", strconv.Itoa(pid), "-Pn").Output()
		if err != nil {
