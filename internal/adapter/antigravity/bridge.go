...
func invokeBridge(ctx context.Context, subcommand, conversationID string, timeout time.Duration, pin bridgePin) (bridgeResult, error) {
	...
	if _, err := exec.LookPath("powershell.exe"); err != nil {
		...
		// cache.
		return bridgeResult{}, fmt.Errorf("powershell.exe not on PATH: %v: %w", err, ErrBridgeTransient)
	}
	...
	if err != nil {
		...
		// drifts.
		return bridgeResult{}, fmt.Errorf("%v: %w", err, ErrBridgeTransient)
	}
	...
	if err := cmd.Run(); err != nil {
		return bridgeResult{}, fmt.Errorf("%s %s: exit %v; stderr: %s", filepath.Base(bridge), subcommand, err, strings.TrimSpace(stderr.String()))
	}
