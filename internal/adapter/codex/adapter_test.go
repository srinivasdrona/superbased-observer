...
func TestParseForkedRolloutSessionMetaOwnership(t *testing.T) {
	...
	assertChildOwnership := func(t *testing.T, res adapter.ParseResult) {
		...
		if got := res.TokenEvents[0].MessageID; got != "turn-child" {
			t.Fatalf("token event message_id = %q; want turn-child", got)
		}
	}
...
func TestIncrementalParseDefersTrailingJSONFragment(t *testing.T) {
	...
	if res2.TokenEvents[0].InputTokens != 12 || res2.TokenEvents[0].OutputTokens != 3 {
		t.Fatalf("resumed token event mismatch: %+v", res2.TokenEvents[0])
	}
}
