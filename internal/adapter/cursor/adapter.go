...
type rawHookPayload struct {
	...
	// After-event outcome fields (v1.4.47). afterShellExecution,
	// afterMCPExecution, postToolUse all carry the result of the
	// preceding tool call. Cursor's exact field names per event aren't
	// pinned in the docs; we accept the conventionally-named variants
	// observed in the reference (success / exit_code / error / output).
	Success  *bool  `json:"success"`   // after*: explicit success flag (preferred)
	ExitCode *int   `json:"exit_code"` // afterShellExecution: 0 ⇒ success
	Output   string `json:"output"`    // after*: output body (used for derived success)
