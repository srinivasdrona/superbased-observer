...
`
	...
	if tok.InputTokens != 15474 {
		t.Errorf("InputTokens = %d, want 15474", tok.InputTokens)
	}
...
`
	...
	} else if te.InputTokens != 5000 || te.OutputTokens != 100 || te.CacheReadTokens != 1000 {
		t.Errorf("hex:colon usage: got in=%d out=%d cr=%d want 5000/100/1000", te.InputTokens, te.OutputTokens, te.CacheReadTokens)
	}
	if te, ok := byMID["00000-abcd1234-ef56-7890-abcd-1234567890ab"]; !ok {
		t.Errorf("missing TokenEvent for 00000-uuid Request-ID")
	} else if te.InputTokens != 6000 || te.OutputTokens != 200 || te.CacheReadTokens != 2000 {
		t.Errorf("uuid usage: got in=%d out=%d cr=%d want 6000/200/2000", te.InputTokens, te.OutputTokens, te.CacheReadTokens)
	}
}
...
func TestParseEventsJSONL_SessionShutdownModelMetrics(t *testing.T) {
	...
	if opus.InputTokens != 578788 {
		t.Errorf("opus InputTokens = %d, want 578788", opus.InputTokens)
	}
...
func TestParseEventsJSONL_CompactionTokensOldSchema(t *testing.T) {
	...
	if te.InputTokens != 131922 {
		t.Errorf("InputTokens = %d, want 131922 (from .input fallback)", te.InputTokens)
	}
...
func TestParseEventsJSONL_CompactionTokensNewSchema(t *testing.T) {
	...
	te := res.TokenEvents[0]
	if te.InputTokens != 150346 || te.OutputTokens != 7690 ||
		te.CacheReadTokens != 142230 || te.CacheCreationTokens != 2048 {
		t.Errorf("token columns wrong: in=%d out=%d cr=%d cc=%d want 150346/7690/142230/2048",
...
func TestParseEventsJSONL_SessionShutdownPartialFields(t *testing.T) {
	...
	te := res.TokenEvents[0]
	if te.InputTokens != 4096 || te.CacheReadTokens != 2048 {
		t.Errorf("captured fields wrong: in=%d cr=%d want 4096/2048", te.InputTokens, te.CacheReadTokens)
	}
