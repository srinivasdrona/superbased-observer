...
func (f *fakeSink) UpdateActionOutcome(ctx context.Context, sourceFile, sourceEventID string, success bool, errorMessage string, durationMs int64) (int64, error) {
	if f.delay > 0 {
		select {
		case <-time.After(f.delay):
		case <-ctx.Done():
...
func (z *zeroRowsSink) UpdateActionOutcome(_ context.Context, _, _ string, _ bool, _ string, _ int64) (int64, error) {
	return 0, nil
}
