...
// Smoke test: tools/list now returns 13 tools (12 spec §11.2 + the
// G33 list_actions_around tool added in v1.4.43+ for three-layer
// progressive disclosure).
func TestServer_ToolsListReturnsThirteen(t *testing.T) {
	s, _, _ := testServer(t)
	resp := rpcCall(t, s, "tools/list", 1, nil)
	tools := resp["result"].(map[string]any)["tools"].([]any)
	if len(tools) != 13 {
		t.Errorf("tools count: %d (want 13)", len(tools))
	}
}
