...
// -----------------------------------------------------------------------------
// retrieve_stashed (CCR — v1.4.41 / Tier 1 / G31)
// -----------------------------------------------------------------------------
...
func newRetrieveStashedTool(s *stash.Stash, signals SignalRecorder) Tool {
	return &retrieveStashedTool{store: s, signals: signals}
}

func (*retrieveStashedTool) Name() string { return "retrieve_stashed" }
func (*retrieveStashedTool) Description() string {
	return "Retrieve the original bytes of a tool_result body that was stashed by the proxy when it exceeded the inline-size threshold. The proxy emits a marker like `[output 47KB stashed at observer://stash/<sha>; use retrieve_stashed]` in place of the body; pass the sha here to fetch the original. Use only when you need the full content — the marker itself often carries enough signal (size, source command) to plan the next step."
}
...
func (*retrieveStashedTool) InputSchema() map[string]any {
	return map[string]any{
		...
		"properties": map[string]any{
			"sha": map[string]any{
				"type":        "string",
				"description": "The 64-char lowercase hex SHA-256 of the stashed body (the `<sha>` portion of the marker URL).",
			},
			"max_bytes": map[string]any{
				"type":        "integer",
				"description": "Optional cap on returned bytes. When set and the stashed body is larger, returns only the first max_bytes bytes plus a `truncated: true` flag.",
			},
		},
		"required": []string{"sha"},
	}
...
func (t *retrieveStashedTool) Invoke(ctx context.Context, raw json.RawMessage) (any, error) {
	...
	body, err := t.store.Read(args.Sha)
	if errors.Is(err, stash.ErrNotFound) {
		return nil, fmt.Errorf("retrieve_stashed: sha %q not found (may have been GCed; re-run the producing tool to regenerate)", args.Sha)
	}
	if errors.Is(err, stash.ErrCorrupt) {
		return nil, fmt.Errorf("retrieve_stashed: sha %q is corrupt on disk; re-run the producing tool", args.Sha)
	}
	if err != nil {
		return nil, fmt.Errorf("retrieve_stashed: %w", err)
	}
	...
	if args.MaxBytes > 0 && len(body) > args.MaxBytes {
		res.Content = string(body[:args.MaxBytes])
		res.Truncated = true
	}
	// K43: log one signal per successful retrieve so the learn
	// pattern miner can surface high-retrieval-rate shas later.
	if t.signals != nil {
		_ = t.signals.RecordRetrieveStashed(ctx, args.Sha, "")
	}
	return res, nil
}
