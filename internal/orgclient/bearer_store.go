...
// rooted at fallbackDir and logs a single WARN naming the downgrade. A nil
// logger is tolerated (slog.Default is used).
func OpenBearerStore(service, fallbackDir string, logger *slog.Logger) BearerStore {
	...
	logger.Warn("org bearer: OS keychain unavailable, falling back to 0600 file store",
		"service", service, "dir", fallbackDir)
	return &fileStore{dir: filepath.Join(fallbackDir, "org-bearer"), service: service}
}
