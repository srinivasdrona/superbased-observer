...
// Returns a zero CostBreakdown when the model is unrecognized
// (matching Compute's "unknown model → $0" semantics).
func (e *Engine) ComputeBreakdown(model string, b TokenBundle) (CostBreakdown, bool) {
	p, ok := e.Lookup(model)
	if !ok {
...
// CostBreakdown splits a TokenBundle's cost into the AI-model portion
...
// need the total.
func ComputeBreakdown(p Pricing, b TokenBundle) CostBreakdown {
