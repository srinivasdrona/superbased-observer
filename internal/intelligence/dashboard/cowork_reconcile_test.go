...
func seedCoworkData(t *testing.T, s *Server, sessionID, model string, tokens models.TokenEvent, totalCostUSD float64) {
	...
	_, err := st.Ingest(context.Background(),
...
func TestCoworkReconcile_HappyPath(t *testing.T) {
	...
	seedCoworkData(t, s, "sess-happy", "claude-opus-4-6",
		models.TokenEvent{
			InputTokens:           1000,
			OutputTokens:          500,
...
func TestCoworkReconcile_OverThreshold(t *testing.T) {
	...
	seedCoworkData(t, s, "sess-drift", "claude-opus-4-6",
		models.TokenEvent{
			InputTokens:           1000,
			OutputTokens:          500,
			CacheReadTokens:       10000,
...
func TestCoworkReconcile_UnknownModelFlagged(t *testing.T) {
	t.Parallel()
	s, _ := newTestServer(t)
	seedCoworkData(t, s, "sess-unknown", "claude-experimental-zzz",
...
func TestCoworkReconcile_EndpointReturnsJSON(t *testing.T) {
	t.Parallel()
	s, _ := newTestServer(t)
	seedCoworkData(t, s, "sess-http", "claude-opus-4-6",
