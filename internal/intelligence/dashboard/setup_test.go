...
// TestSetupClaude_MethodNotAllowed — POST/DELETE return 405 with Allow.
func TestSetupClaude_MethodNotAllowed(t *testing.T) {
	withFakeClaudeHome(t, "")
	srv := newSetupServer(t, 8820)
	rr := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/api/setup/claude", nil)
	srv.Handler().ServeHTTP(rr, req)
	if rr.Code != http.StatusMethodNotAllowed {
		t.Errorf("POST: got %d want 405", rr.Code)
	}
	if got := rr.Header().Get("Allow"); !strings.Contains(got, "GET") {
