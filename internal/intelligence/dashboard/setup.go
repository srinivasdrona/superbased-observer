...
// claudeSetupSnapshot is the GET /api/setup/claude response. The Claude
// card is purely informational (no config to write — `observer claude`
// is a launcher, not a config-mutation step), so there's no POST and
// no force/dry_run dance. The frontend renders the snippet and a
// status pill on the Compression tab.
...
// handleSetupClaude serves /api/setup/claude (GET only). Returns the
// current Claude Code install state relative to this observer's proxy
// port plus the launcher command the user should run.
