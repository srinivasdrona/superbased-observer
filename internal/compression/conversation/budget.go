...
type BudgetOptions struct {
	...
	//   - the block named in PreserveBlockIdx (the SDK's cache_control
	//     marker block) is skipped from per-type compression as a
	//     safety net.
	CacheAware bool
	// PreserveBlockIdx names the (msg, block) pair to leave bit-
	// identical in CacheAware mode. When .Msg is -1 the option is
	// inactive. Populated by the pipeline from
	// [findLastCacheBreakpoint].
	PreserveBlockIdx struct {
		Msg   int
		Block int
	}
}
