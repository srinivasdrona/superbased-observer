...
// surround it.
//
// preserveMsg/preserveBlock, when both ≥ 0, name the cache_control
// marker block to leave bit-identical (cache-aware mode safety net —
// the marker block goes into Anthropic's new cache write, and the
// trade-off of leaving it uncompressed is worth one variable removed
// from the determinism story). Pass -1, -1 to disable preservation.
func compressToolResults(extracted []extractedMessage, registry *Registry, allow map[types.ContentType]bool, preserveMsg, preserveBlock int) []Event {
	if registry == nil {
		return nil
	}
	var events []Event
	for i := range extracted {
		...
		for j, body := range em.resultTexts {
			...
			if i == preserveMsg && em.resultBlockIdxs[j] == preserveBlock {
				continue
			}
...
func substituteRedundantReads(extracted []extractedMessage, sessionID string, preserveMsg, preserveBlock int) []Event {
	if sessionID == "" {
		return nil
	}
...
// intact.
//
// preserveMsg/preserveBlock honour the same protection
// [compressToolResults] applies — the cache_control marker block is
// left bit-identical even if it's > threshold.
//
...
}, threshold int, preserveMsg, preserveBlock int) []Event {
