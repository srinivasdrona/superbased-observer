"""
Gate 2.3 SWE-agent runner — 3× variance-estimated re-run.
...
Usage (from Windows PowerShell with WSL2 active):
  python compression-testing/gate2/run_gate2_3.py [--batch N] [--resume]
...
logger = logging.getLogger("gate2.3")
...
def main():
    ...
    output_root = Path(__file__).parent / "runs" / f"gate2_3_swe_{run_id}"
    output_root.mkdir(parents=True, exist_ok=True)
    state_path = output_root / "state.json"

    # Resume support: if --resume and output_root already has a run_id in env
    resume_root_env = os.environ.get("GATE23_RESUME_DIR")
    if args.resume and resume_root_env:
        output_root = Path(resume_root_env)
        state_path = output_root / "state.json"
        logger.info(f"Resuming run at {output_root}")
    else:
        logger.info(f"New Gate 2.3 run: {output_root}")
    ...
    for batch_idx in batch_indices:
        ...
        for rep in range(1, N_REPS + 1):
            for arm, _ in ARMS:
                ...
                harness_run_id = f"gate2-3-b{batch_idx+1:02d}-r{rep}-{arm}"
                if n_patched == 0:
