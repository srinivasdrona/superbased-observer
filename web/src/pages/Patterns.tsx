          ...
          <div className="flex h-[180px] items-end gap-1 overflow-x-auto pb-1">
            {days.map((p) => {
              return (
                <div
                  key={p.day}
                  className="flex h-full min-w-[14px] flex-col items-stretch justify-end"
                  title={`${p.day} · ${fmtInt(p.total)} reinforcements`}
                >
                    ...
                    {types.map((t) => {
                      ...
                      return (
                        <span
                          ...
                          title={`${PATTERN_LABEL[t] ?? humanize(t)}: ${n}`}
                        />
                      );
                    })}
                  </div>
    ...
    >
      <header className="flex items-baseline justify-between gap-2">
        <span className="flex items-center gap-2 min-w-0">
          ...
          <span
            className="truncate font-mono text-[10.5px] text-fg-3"
            title={row.project}
          >
            {row.project ? shortProject(row.project) : (
        ...
        <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5 rounded-2 bg-bg-1 px-2 py-1.5 font-mono text-[11px]">
          {kvs.map(([k, v]) => (
            <Fragment key={k}>
              ...
              <dd
                className="min-w-0 truncate text-fg-1"
                title={String(v)}
              >
                {formatVal(v)}
              </dd>
    ...
    return (
      <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5 rounded-2 bg-bg-1 px-2 py-1.5 font-mono text-[11px]">
        {kvs.map(([k, v]) => (
          <Fragment key={k}>
            <dt className="text-fg-3">{k}:</dt>
            <dd className="min-w-0 truncate text-fg-1" title={String(v)}>
              {formatVal(v)}
            </dd>
          </Fragment>
        ))}
...
function FlowNode({
  ...
  return (
    <div className="min-w-0 flex-1">
      ...
      <div
        className="mt-0.5 truncate text-fg-1"
        title={value || "(unset)"}
      >
