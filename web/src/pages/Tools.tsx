              ...
              <div className="flex h-3 w-full overflow-hidden rounded-pill bg-bg-3">
                {sorted.map(([type, n]) => {
                  ...
                  return (
                    <span
                      key={type}
                      title={`${meta.label}: ${fmtInt(n)} (${fmtPct(n / Math.max(1, t.total))})`}
                      style={{
                        width: `${pct}%`,
          ...
          {rows.map((t) => {
            ...
            return (
              ...
              >
                ...
                <td className="py-1.5 text-fg-3" title={t.first_seen}>
                  {fmtCompactDate(t.first_seen)}
                </td>
                <td className="py-1.5 text-fg-3" title={t.last_seen}>
