...
function buildColumns(showScoring: boolean): ColumnDef<SessionRow, unknown>[] {
  ...
  const cols: ColumnDef<SessionRow, unknown>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) =>
        row.original.project ? (
          <span
            className="block max-w-[240px] truncate font-mono text-[11px] text-fg-3"
      ...
      cell: ({ row }) => {
        ...
        return (
          ...
          >
            ...
            {extras.length > 0 && (
              <span
                className="shrink-0 font-mono text-[10.5px] text-fg-3"
    ...
    },
    {
      ...
      meta: { align: "right", mono: true },
      cell: ({ row }) =>
        row.original.cache_creation_tokens > 0 ? (
          <span className="tabular-nums text-fg-1">
            {fmtCompact(row.original.cache_creation_tokens)}
          </span>
        ) : (
          <span className="text-fg-4">—</span>
        ),
  ...
  if (showScoring) {
    cols.push(
      ...
      },
      {
        ...
        meta: { align: "right" },
        cell: ({ row }) =>
          row.original.redundancy_ratio != null
            ? fmtPct(row.original.redundancy_ratio)
            : "—",
      },
    );
  }
