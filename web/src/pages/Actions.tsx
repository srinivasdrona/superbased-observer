    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <span title={row.original.timestamp} className="text-fg-2">
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span
            className="block max-w-[240px] truncate font-mono text-[11px] text-fg-2"
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span
              className="max-w-[180px] truncate font-mono text-[10.5px] text-fg-3"
              title={sf}
            >
              {tail}
            </span>
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.excerpt ? (
            <span
              className="block max-w-[200px] truncate font-mono text-[10.5px] text-fg-2"
        ...
        cell: ({ row }) => (
          <span className="inline-flex items-center gap-1.5">
            <button
              ...
              title={`Open session ${row.original.session_id}`}
            >
    ...
    <div className="rounded-3 border border-line-2 bg-bg-2 px-3 py-2.5">
      ...
      <div className="flex items-end gap-1 overflow-x-auto pb-1">
        <button
          ...
          title="Show the most-recent actions across all days in the window"
        >
        ...
        {cells.map((c) => {
          ...
          return (
            <button
              ...
              title={`${c.day} · ${c.count} action${c.count === 1 ? "" : "s"}`}
            >
              <span
                className={clsx(
                  "w-[6px] rounded-pill",
...
function TimelineEntry({
  ...
  return (
    <li className="flex items-stretch gap-3 py-2">
      <div
        className="shrink-0 pt-2 text-right font-mono text-[10.5px] tabular-nums text-fg-3"
        style={{ width: 80 }}
    ...
    <article className="rounded-3 border border-line-2 bg-bg-1 p-3.5 transition-colors hover:border-line-3">
      <header className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          ...
          {row.raw_tool_name && (
            <span
              className="max-w-[180px] truncate font-mono text-[11px] text-fg-1"
              title={row.raw_tool_name}
            >
              {row.raw_tool_name}
            </span>
          )}
      ...
      <footer className="mt-2 flex flex-wrap items-center gap-1.5 text-[10.5px] text-fg-3">
        ...
        {sf && (
          <span
            className="ml-auto max-w-[200px] truncate font-mono text-[10.5px] text-fg-4"
            title={sf}
          >
            {sfTail}
          </span>
...
function ActionTypeBadge({ type }: { type: string }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-1 rounded-pill border px-2 py-0.5 text-[10.5px] font-medium"
      style={{
        borderColor: `color-mix(in srgb, ${meta.colorVar} 35%, transparent)`,
        background: `color-mix(in srgb, ${meta.colorVar} 12%, transparent)`,
        color: meta.colorVar,
      }}
      title={`${meta.label} · category: ${meta.category}`}
    >
      <span
...
function DetailRow({
  ...
  return (
    <div className="flex gap-2">
      ...
      <dd
        className={`min-w-0 truncate ${mono ? "font-mono text-fg-2" : "text-fg-1"}`}
        title={String(value)}
      >
        {value}
      </dd>
