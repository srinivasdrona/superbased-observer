  ...
  return (
    <div className="space-y-6 p-6">
          ...
          <div className="flex items-center gap-2">
            {(models.data?.unknown_model_count ?? 0) > 0 && (
              <Link
                ...
                title="Open Settings → Pricing to add overrides"
              >
            ...
            <button
              ...
              title="Download per-model cost table as CSV"
            >
              Export
            </button>
          </div>
      ...
      >
        ...
        >
          {models.data && <ModelTable rows={models.data.rows} />}
        </ChartState>
      </ChartShell>
...
function ModelTable({ rows }: { rows: CostSummary["rows"] }) {
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[1240px] text-left text-[11.5px]">
        ...
        <tbody>
          {rows.map((r, i) => (
            <ModelRow key={r.key} row={r} zebra={i % 2 === 1} />
          ))}
        </tbody>
      </table>
    </div>
