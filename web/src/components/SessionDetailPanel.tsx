...
export function SessionDetailPanel({
  ...
  //   - "turn" (default): one message row per user-turn (claudecode
  //     msg.ID for Anthropic, codex turn boundary). Tokens for all
  //     model inferences within a turn are summed into that row.
  ...
  // Reset to "turn" + page 1 when the operator navigates to a
  // different session so each new panel opens in the rollup view.
  const [tokenDetail, setTokenDetail] = useState<"turn" | "inference">("turn");
  useEffect(() => {
    setMsgPage(1);
    setTokenDetail("turn");
  }, [sessionId]);
          ...
          <h3 className="flex items-center justify-between gap-2">
            ...
            <span className="flex items-center gap-3">
              {/* Group toggle is meaningful for codex (v1.7.24+ emits
                  one token row per model inference). For Anthropic
                  adapters (claudecode et al.) "inference" is identical
                  to "turn" — the watcher already emits one row per
                  upstream msg_xxx — so the toggle is a no-op there. */}
              {detail.data?.tool === "codex" && (
                <Tooltip content="Group token rows by user-turn (default) or by model inference (one row per codex token_count event). Tool calls always stay grouped at the turn level.">
                  <span>
                    <SegmentedControl
                      size="sm"
                      value={tokenDetail}
...
function CachePanel({
  ...
  return (
    <section className="mt-5 space-y-2">
      ...
      <ForecastWidget sessionId={sessionId} />
    </section>
  );
}
