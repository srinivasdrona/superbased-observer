      ...
      <div className="relative">
        ...
        {hasSplit ? (
          <div className="mt-1 grid grid-cols-2 gap-x-3 gap-y-0.5 text-[10.5px]">
            ...
            <span
              className="text-right font-mono tabular-nums text-fg-1"
              title={fmtUSD(d.ai_cost_usd, true)}
            >
              {fmtUSD(d.ai_cost_usd)}
            </span>
            <span className="text-fg-3">Tool</span>
            <span
              className="text-right font-mono tabular-nums text-fg-2"
              title={fmtUSD(d.tool_cost_usd, true)}
            >
              ...
              return (
                <li key={r.model} className="space-y-1">
                  <div className="flex items-baseline justify-between gap-2">
                    ...
                    <span
                      className="shrink-0 font-mono tabular-nums font-semibold text-fg-0"
                      title={
                        mode === "cost"
      ...
      <table className="w-full min-w-[1260px] text-left text-[11px]">
        <thead className="bg-bg-3/40 text-[10px] uppercase tracking-[0.06em] text-fg-3">
          <tr className="whitespace-nowrap border-b border-line-2">
            ...
            <th className="py-1.5 font-medium" title="Reasoning effort — codex collaboration_mode.settings.reasoning_effort, antigravity SKU-encoded low/medium/high, etc. Empty for adapters that don't expose an effort knob (Anthropic models, copilot, etc.).">Effort</th>
            ...
            return (
              <Fragment key={key}>
                ...
                >
                  ...
                  <td
                    className="max-w-[140px] truncate py-1 font-mono text-fg-2"
                    title={m.model || ""}
                  >
                    {m.model || "—"}
                  </td>
                  ...
                  <td className="py-1 text-right tabular-nums text-fg-2">
                    {hasToolCalls ? (
                      <span
                        ...
                        title="Click row to expand tool calls"
                      >
                  ...
                  <td
                    className="py-1 text-right tabular-nums text-fg-1"
                    title={m.ai_cost_usd > 0 ? fmtUSD(m.ai_cost_usd, true) : ""}
                  >
                    {m.ai_cost_usd > 0 ? fmtUSD(m.ai_cost_usd) : "—"}
                  </td>
                  <td
                    className="py-1 text-right tabular-nums text-fg-3"
                    title={m.tool_cost_usd > 0 ? fmtUSD(m.tool_cost_usd, true) : ""}
                  >
                    {m.tool_cost_usd > 0 ? fmtUSD(m.tool_cost_usd) : "—"}
                  </td>
                  <td
                    className="py-1 text-right tabular-nums text-fg-0"
                    title={m.cost_usd > 0 ? fmtUSD(m.cost_usd, true) : ""}
                  >
                    {m.cost_usd > 0 ? (
                      <span className="font-semibold">{fmtUSD(m.cost_usd)}</span>
                    ) : (
                      "—"
                    )}
                  </td>
    ...
    >
      ...
      {tc.excerpt && (
        <CopyOnClick
          ...
          title="Click to copy full output"
        >
