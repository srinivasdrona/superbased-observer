#!/usr/bin/env bash
# ab-codex-report.sh — pull headline numbers from both codex A/B
# observer DBs and print a markdown table comparing the two runs.
# Mirrors ab-claude-report.sh, filtered to provider = 'openai'.
#
# Reads /tmp/ab-codex/{on,off}/observer.db; needs sqlite3 in PATH.
#
# Default: only rows newer than the run-start marker written by
# ab-codex-start.sh. Override via --since=<ISO-timestamp> or --all.
#
# Output goes to stdout. Pipe into the report doc:
#   ./scripts/ab-codex-report.sh > docs/codex-compression-ab-results.md
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      ...
      echo "  Default: filter to rows newer than $MARKER (set by ab-codex-start.sh)"
...
for db in "$ON_DB" "$OFF_DB"; do
  if [[ ! -f "$db" ]]; then
    echo "$db not found — run scripts/ab-codex-setup.sh + ab-codex-start.sh + actually drive a codex session first" >&2
    exit 1
  fi
done
...
cat <<MD
# Codex CLI compression A/B — generated $(date -u +%Y-%m-%dT%H:%M:%SZ)
...
WITH ranked AS (
  SELECT total_response_ms,
         ...
         COUNT(*) OVER () AS n
  FROM api_turns
  WHERE provider = 'openai' AND total_response_ms IS NOT NULL$since_clause
)
...
Generated by \`scripts/ab-codex-report.sh\`. Re-run after each codex
session to refresh the numbers; the script reads both DBs read-only.
MD
