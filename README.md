...
binary; `scp` it anywhere it runs.

Pre-built binaries ship for Linux x64/arm64, macOS Intel/Apple
...
# 3. Run watcher daemon + dashboard + (optionally) proxy.
...
Once registered (`observer init`), every connected AI tool can
query the observer over MCP/stdio:
...
1. **Forwards your request verbatim** to Anthropic or OpenAI — no
   data leaves your machine that wasn't already going to the
   provider.
2. **Records the exact token counts** the provider returned (cache
   5m vs 1h split, long-context tier triggers, reasoning tokens) —
   more accurate than parsing the JSONL the AI tool wrote.
3. **Optionally compresses** the request body before forwarding
   (importance-scored, prefix-stable for cache alignment) — savings
...
- **Shell output filters** — RTK-style truncation of large `bash` /
  ...
  outputs inline before they hit the LLM context.
...
- **Conversation compression** — proxy rewrites large `tool_result`
  blocks before forwarding upstream. Off by default; opt-in via
  `[compression.conversation].enabled = true`.
